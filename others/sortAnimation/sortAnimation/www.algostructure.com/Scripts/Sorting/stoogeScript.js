...
function fastKey(){
    ...
    function graphics(){
    	...
    	ctx.fillStyle = "white";
    	ctx.fillRect(this.x, this.bottom - this.key * 300 / maxKey, 10, 2);
    	ctx.fillText(this.key, this.x, this.bottom - 10 - this.key * 300 / maxKey);
    }
}
...
// sorts all keys with stoogeSort.
function stoogeSort() {
    try {
        j = stack.pop();
        i = stack.pop();
        ...
        if (arr[j].key < arr[i].key) {
            setTimeout(function () {
                swapKeys(arr[j], arr[i]);

                continueStoogeSort();
            }, milSec);
        }
        else {
            setTimeout(function () { continueStoogeSort(); }, milSec);
        }
    } catch (err) {
        if (stack.length <= 0) {
